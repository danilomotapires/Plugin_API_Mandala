...
function teste2_api(){
	...
	$response_cs = wp_remote_get('https://sportsdata.io/developers/api-documentation/csgo#/endpoint/schedule/100000138', 'Ocp-Apim-Subscription-Key: {Oda786O1f9584Oe8c6311d6f7f9f181}');

	if(is_array($response_cs)){
		print_r("deu bom");

		$cabecalho 	= $response_cs['headers'];
		$body		= $response_cs['body'];
		$dados		= json_decode($body);

		print_r($dados);
	}
}
add_action('wp_head', 'teste2_api');
